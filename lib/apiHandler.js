...
export function parseQueryOptions(searchParams) {
  ...
  for (const [key, value] of searchParams.entries()) {
    ...
    if (key.endsWith("_is_null")) {
      ...
      options.filters[fieldName] = null; // Use null as filter value
      options.operators[fieldName] =
        value === "true" ? "IS NULL" : "IS NOT NULL";
      continue;
    }

    // Handle IS NOT NULL checks (e.g., sub_time_is_not_null=true)
    if (key.endsWith("_is_not_null")) {
      const fieldName = key.replace("_is_not_null", "");
      options.filters[fieldName] = null; // Use null as filter value
      options.operators[fieldName] =
        value === "true" ? "IS NOT NULL" : "IS NULL";
      continue;
    }

    // Handle comparison operators (gt, gte, lt, lte, ne, like)
  ...
  try {
    ...
    if (id) {
      ...
      if (Object.keys(options.filters).length > 0) {
        const deletedRows = await crud.removeByFilters(table, options.filters, options.operators);
        return NextResponse.json({ success: true, deleted: deletedRows });
      }
      return NextResponse.json({ error: "Missing id or filters" }, { status: 400 });
    }
  } catch (err) {
