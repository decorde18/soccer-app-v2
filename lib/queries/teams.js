...
export const getStatLeaders = cache(async (teamSeasonId) => {
  const [rows] = await pool.query(
    `SELECT * FROM v_stat_leaders WHERE team_id = ?`,
    [teamSeasonId],
  );
  return rows[0] || null;
});
...
export const getUserTeams = cache(async (userId, userRole = null) => {
  ...
  if (userRole?.systemAdmin === true || userRole?.systemAdmin === 1) {
    const [rows] = await pool.query(
      ...
      WHERE ts.is_active = 1 AND t.is_active = 1 AND c.is_active = 1 AND s.status='active'
      ORDER BY s.start_date DESC, c.name, t.team_name;`,
    );
    return rows;
  }
