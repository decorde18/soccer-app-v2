...
export default async function ParentLayout({ children }) {
  ...
  if (!token) {
    redirect("/auth/login?returnUrl=/parent");
  }
  ...
  // Check permissions
  if (hasRoleAccess(user.roles, "parent") === false) {
    // Return 403 component
    return (
      <div className='flex min-h-screen items-center justify-center'>
        <div className='text-center'>
          <h1 className='text-2xl font-bold mb-2'>Access Denied</h1>
          <p className='text-gray-600 mb-4'>Parent access required</p>
